...
parentPort?.on('message', async (userData) => {
  try {
      const { id, partnerIdx, firstIdx, data, address, year, month, type, detail, owner, spender, amount } = userData;
        if(data) {
            ...
            parentPort?.postMessage(result);
        } else if (type === 'getIplHistory') {                                 
            const result = await getIplHistory(firstIdx);
            parentPort?.postMessage(result);
        }
...
async function updateAddressBatch() {
    ...
    try {
        ...
        const TEL_ADDRESS_URL = configService.get<string>('');
...
async function getVestingProof(address: string): Promise<any> {
        ...
        try {
            const monthlySnapshotData = await db.snapshotInfoRepo
                                                    ...
                                                    .where('monthlySnap.month LIKE :month', {month: `${snapMonth}%`})
                                                    .getMany();
            for (const snap of monthlySnapshotData) {
                console.log(`monthlySnapshotData round_id ::: ${snap.round_id}`);
                const vestingData = await db.vestingClaimsRepo
                                            ...
                                            .andWhere('vc.round_id = :round_id', { round_id: snap.round_id })
                                            .getOne();
                if(vestingData) {
...
async function getSnapshot(address: string): Promise<any> {
        ...
        try {
            ...
            logger.log('SnapshotData Nothing', e);
            return { success: true, data: 'coming soon' };
        }
    }
...
async function getSnapshotBalance(year: string, month: string): Promise<any> {
        ...
        while(true) {
            ...
            const whitelistMap = new Map<string, string>(
                whitelistInfoList.map(w => [w.address, w.principal])
            );
        ...
        if (!snapshotRow) {
          ...
          logger.log(`getIplBalance jsonData ::: ${data}`);
          if(jsonData) {
                for (const [address, info] of Object.entries(jsonData) as [string,{ balance: string; share: string }][]) {
                    ...
                    const principal = whitelistMap.get(address);
                    
                    console.log(principal);

                    const getAddress = await db.userBackupRepo
                                                .createQueryBuilder('userbackup')
                                                .where('userbackup.principal = :principal', { principal: principal })
                                                .getOne();

                    if(!getAddress) {
                        logger.log('evmAddress Data nothing.');
                        continue;
                    }
                    ...
                    if(balance === '0' || share === '0' || getAddress.evmAddress === '0x0000000000000000000000000000000000000000' || !ethers.isAddress(getAddress.evmAddress)) {
                        ...
                        logger.log(`getIplBalance save ::: ${getAddress.evmAddress} ${balance} ${share} ${amount}`);
    ...
    async function getIplHistory (firstIdx: number): Promise<any> {
        const endIdx = 14494863;
        const batchSize = Number(process.env.IPL_HISTORY_BATCH_SIZE || 5000);
        let totalMissing = 0;

        while (firstIdx < endIdx) {
            const start = firstIdx;
            const end = Math.min(firstIdx + batchSize - 1, endIdx - 1);
            logger.log(`getIplHistory batch ${start} ~ ${end}`);
            ...
            const isZeroAddress = (addr: any) => {
                if (!addr) return true;
                const normalized = String(addr).toLowerCase().trim();
                return normalized === '0x0000000000000000000000000000000000000000';
            };

            for (const i of indices) {
                const row = jsonData[i];
                if (!row || isZeroAddress(row.account)) {
                    totalMissing += 1;
                    logger.log(`getIplHistory missing web3 ::: ${i} ${row?.account ?? 'null'}`);
                } 
            }

            firstIdx = end + 1;
        }
        return { success: true, totalMissing };
      }
  ...
  async function getPrincipalById(partnerIdx: number, id: string) {
              ...
              const results = await Promise.all(
                  partnerIdxList.map(async (idx) => {
                      ...
                      for (const m of merged) {
                          logger.log(`merged ::: ${m.principle} ${Number(m.partner_idx)} ${m.created_at}`); 
                        if(idx === 0) {
                            await db.userBackupRepo.save({
                                id: pid,
                                evmAddress: id,
      ...
      async function addPrincipal(id: any, partnerIdx: number) {
        ...
        function safeDecrypt(id: any) {
            if (id == null) return id; // null/undefined 그대로 반환

            try {
                const decrypted = telegramService.decrypt(id);
                return decrypted ?? id; // decrypt 결과가 null이면 원본 반환
            } catch (e) {
                return id; // 복호화 실패 시 원본 그대로 사용
            }
        }
