...
import { IplMintHistory } from 'src/entity/iplminthistory.entity';
...
export async function userDBService() {
  ...
  const UserDataSource = new DataSource({
    ...
    const VaultDataSource = new DataSource({
      ...
      entities: [IplWhitelist, IplSnapshot, IplMintHistory, IplMonthlySnapshots, VestingClaims, VestingWhitelists, VestingHistoryLog, VestingWhitelistsTemp],
    });
  ...
  const iplMintHistoryRepo: Repository<IplMintHistory> = VaultDataSource.getRepository(IplMintHistory);
  ...
  return { UserDataSource, VaultDataSource, userBackupRepo, whiteListRepo, snapshotRepo, snapshotInfoRepo, iplMintHistoryRepo, vestingClaimsRepo, vestingWhitelistRepo, vestingHistoryRepo, vestingWhitelistTempRepo };
}
